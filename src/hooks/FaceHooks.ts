...
const useFaceDetection = () => {
      ...
      const getDescriptors = async (videoRef: RefObject<HTMLVideoElement>) => {
        ...
        const result = await faceapi.detectSingleFace(videoRef.current, new faceapi.TinyFaceDetectorOptions()).withFaceLandmarks(true).withFaceDescriptor();
        if (!result) {
    ...
    const matchFace = async (currentDescriptors: Float32Array, descriptorsFromDB: Float32Array[]) => {
      if(descriptorsFromDB && descriptorsFromDB.length > 0) {
        const faceMatcher = new faceapi.FaceMatcher(descriptorsFromDB.map((descriptor) => {
